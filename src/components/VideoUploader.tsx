...
export const VideoUploader: React.FC<VideoUploaderProps> = ({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      setError((err as Error)?.message || 'Failed to read video file. Please try standard MP4 or WebM.');
    } finally {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-6 max-w-3xl mx-auto w-full">
      {/* Hidden file input outside the card to prevent event bubbling anomalies */}
      <input
        ref={fileInputRef}
        type="file"
      ...
      >
        ...
        <div>
          <h3 className="text-lg font-semibold text-slate-100 mb-1">
            {isProcessing ? 'Reading Video Metadata...' : 'Drag & Drop Your Short Video Clip Here'}
          </h3>
          <p className="text-sm text-slate-400 max-w-md mx-auto">
            Supports MP4, WebM, MOV, and M4V. Optimized for short video clips with watermarks in the bottom-right corner.
          </p>
        </div>
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-lg bg-sky-600 hover:bg-sky-500 text-white font-medium text-sm transition-all shadow-md shadow-sky-600/25 disabled:opacity-50 flex items-center gap-2"
        >
          {isProcessing && <Loader2 className="w-4 h-4 animate-spin" />}
          <span>{isProcessing ? 'Processing Video...' : 'Select Video File'}</span>
        </button>
      </div>

      {error && (
        <div className="mt-4 flex items-center gap-2 px-4 py-2.5 rounded-lg bg-red-500/10 border border-red-500/30 text-red-300 text-sm max-w-xl">
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                4s clip featuring a bottom-right logo & watermark badge
              </p>
            </div>
          </button>
